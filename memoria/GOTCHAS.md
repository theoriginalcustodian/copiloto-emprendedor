...
> **NOTA:** Este archivo se desprendió de MEMORY.md para reducir Context Bloat.
> Contiene todas las lecciones pasadas. Consultar por nombre/tópico si ocurre un bug extraño.
...
- [🈳🟢 El chequeo de tipos que compilaba el proyecto VACÍO](el-chequeo-de-tipos-que-compilaba-el-proyecto-vacio.md) — `project`. `tsc --noEmit` sobre un tsconfig de referencias sale **exit 0 sin mirar un archivo**. Preguntá el DENOMINADOR, no si pasó.
- [📐🚫 Una tabla IGNORA el `max-width` de su celda](una-tabla-ignora-el-max-width-de-su-celda.md) — `project`. Dos defectos, suite verde en ambos: **jsdom no hace layout**. Receta del gate visual.
