...
**How to apply:** al cerrar CUALQUIER trabajo no trivial, propagar el estado a **TODOS** los docs maestros, no solo a `§5` + memorias: `CLAUDE.md §5` + `docs/ROADMAP.md` + `docs/ARCHITECTURE.md` + memoria + reporte de cierre. La reconciliación es mecánica — `§5` + `git log --since=<última reconciliación>` son la verdad-base (no inventar números de PR). Verificar adversarialmente contra el git log (sin overclaim, sin residuos stale, PR# correctos). `HANDOFF.md` el operador lo despriorizó (2026-06-22) → opcional. Señal de que se viene drift: varios PRs cerrados con "actualicé §5 + memoria" sin tocar ROADMAP/ARCHITECTURE. Relacionado: [[kit-canonico-skeleton]] · [[seniorworkflow-durable-sp7]].
