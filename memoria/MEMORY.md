...
- [🪞 El guard se satisface con su PROPIO comentario](el-guard-se-satisface-con-su-propio-comentario.md) — `feedback`. Exigía `min-height: 0` y pasaba con la línea BORRADA: el comentario que la explica contiene la cadena. Cuanto mejor el comentario, más mudo el guard. Todo guard de texto descarta comentarios y se corre en diferencial, en cada dirección.
- [🔢🎭 Contar un símbolo no dice en qué ROL aparece](contar-un-simbolo-no-dice-en-que-rol-aparece.md) — `feedback`. Declaré "prod corre el rail viejo" por 4 keys que estaban en el TIPO, no en el registro. Mismo error 3× en una hora: un conteo devuelve un número igual de firme esté bien o mal formulada la pregunta. Contá la FORMA (`key:"x",label:"…"`), y diferencial contra el artefacto previo.
- [🔇🔨 Mudo ≠ parado — el silencio mide REPORTE, no TRABAJO](mudo-no-es-parado-el-silencio-mide-reporte-no-trabajo.md) — `feedback`. Reasigné el contrato de una sesión viva → 2 PR con el mismo contenido, 6 conflictos. El instrumento imprimía `PRODUCCIÓN 1min` y «NO es dead-man» en la MISMA corrida: cité la señal que confirmaba mi sospecha y saltée la otra. Protocolo: `COORDINACION.md` §4.2.nonies.
...
- [🌳🕳️ El working tree COMPARTIDO guarda trabajo que no está en ninguna rama](el-working-tree-compartido-guarda-trabajo-que-no-esta-en-ninguna-rama.md) — `project`. **Merge-base del 24-jul: 237 commits y 697 archivos de retraso.** Pierde hacia adelante (nunca llega a `main`, ni al grafo) y hacia atrás (sirve herramientas viejas). `??` NO significa perdible: el control es el **blob** (`git cat-file -e $(git hash-object f)`) — bajó "370 cambios" a **2** reales. Rescatar = merge dirigido, nunca copia: la rama está atrasada.
...
- [📱🔀 Metro sirve el bundle del CHECKOUT COMPARTIDO, no del worktree](metro-sirve-el-bundle-del-checkout-compartido-no-del-worktree.md) — `project`. Verificás en device, no ves tu cambio y dudás del código: el bundle venía de otro lado. Confirmá de dónde sirve Metro ANTES de buscar el bug.
...
- [📱🤖 `adb` no puede ejercitar el toque corto de un `Gesture.Pan()`](adb-no-puede-ejercitar-el-toque-corto-de-un-gesture-pan.md) — `project`. `input tap`/`swipe` de 0-2px nunca dispara el toggle, pero taps simples y drags de 600px sí. Sin ese control se concluye "el toque no funciona" y se busca el bug donde no está.
- [📦🔌 Metro no resuelve `node_modules` symlinked en un worktree (Windows)](metro-no-resuelve-node_modules-symlinked-worktree.md) — `project`. 404 `UnableToResolveError` aunque el archivo exista y `tsc`/`jest` resuelvan igual. `unstable_enableSymlinks` NO lo arregla.
- [🕐💥 El cron de backup de Graphity tumba su API 4× al día, 60-90 s](graphity-backup-cron-tumba-el-api-4x-dia-60-90s.md) — `project`. 03:30/09:30/15:30/21:30 local: el `pre-push` fail-closed aborta el push con 503. No es degradación real; `/health` puede seguir en 200.
- [💾⏸️ Backups off-site de fusion y Temporal: implementados y APAGADOS por diseño](backups-fusion-y-temporal-apagados-por-diseno-deuda-diferida.md) — `project`. WAL-G + Backblaze B2 listos; se encienden cuando el operador lo pida. Deuda diferida, no gap.
- [📧⏸️ SMTP y reset de password diferidos por decisión del operador](smtp-email-transaccional-diferido-reset-password.md) — `project`. GoTrue con `MAILER_AUTOCONFIRM=true`; el slot quedó cableado para Gmail SMTP. BETA-2.b/2.c pospuestos, no olvidados.
