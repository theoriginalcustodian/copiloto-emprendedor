...
- [🔑🚫 El copiloto NO tiene admin de Graphity — el provisioning depende de si una key común alcanza](graphity-copiloto-sin-admin-provisioning-gap.md) — `project`. **LEER antes de asumir provisioning.** 403 en `/admin`. Mi runbook asumió admin (error). La pregunta al canal decide todo el camino. Flag: `SALDA` cobro→comprobante cierra "¿quién me debe?".
