...
**Patron de validacion:** editar local → `scp` el archivo + el test al VPS → `pytest` en el venv correspondiente → si verde, deploy (restart del service). **No declarar un test verde sin haberlo corrido en el VPS** (regla 5 del proyecto: evidencia, no autoeval). Casos reales cazados solo en el VPS: `pytest.approx` con `>=` (TypeError, SP6) · `object.__new__(WorkflowAlreadyStartedError)` no-safe (SP7) · `>= pytest.approx(0.60)` desactualizado. Relacionado: [[sp6-observabilidad]], [[roadmap-palancas]].
