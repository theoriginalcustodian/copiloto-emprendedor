...
**How to apply:** NO incluir el MCP `composio` en el HOME de ningún agente autónomo no supervisado de la fábrica (Hermes, Claude Code headless del `FeatureWorkflow`). Como está en scope user, podría filtrarse al entorno de esos agentes — verificar el HOME dedicado de la fábrica ([[casa-fabrica-features-diseno]] ya contempla HOME aislado). Si la fábrica necesitara Gmail, crear un server Composio con allowlist restringido (`GMAIL_FETCH_*`), no el endpoint universal. La API key de Composio es secreto de máxima criticidad (da acceso a borrar el correo). Relacionado: [[canal-whatsapp-hermes]] (Hermes como agente que podría heredar el scope).
