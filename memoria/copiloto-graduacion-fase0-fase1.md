...
**Decisiones LOCKEADAS:** motor = vendorizar-con-sync (Fase 2) → **FORK DURO desde 2026-07-07** (evoluciona en ESTE repo, `sync-motor.sh` retirado) · `dispatcher_emprendedor` queda **divergente** del genérico R1 (deuda visible) · `fabrica-local-containerizada` **diferida** (no es del copiloto; única rama con código real sin mergear) · historia = **filter-repo** en Fase 2.
