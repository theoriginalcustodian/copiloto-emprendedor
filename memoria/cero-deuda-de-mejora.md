...
**Caso fundacional:** el sprint de hardening de la clínica reveló 3 mejoras "por construcción" (template de hardening reusable `hardening_reference.sql` · check `undriven_seam_candidate` en `plan_verifier` · pipeline adversarial obligatorio en `/generar-plano`) que primero quedaron como "follow-up priorizado"; el operador corrigió: se implementan al cerrar. [[clinica-hardening-3-frentes]] [[cero-deuda-no-gestionada]] [[cierre-del-aprendizaje-no-opcional]]
