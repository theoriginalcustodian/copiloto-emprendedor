...
describe('PortadaNegocio — fecha de corte y variación (K-03, BL-J2/BL-J3)', () => {
  ...
  it('variación null (un solo mes de historia): sólo la fecha, sin «0%» ni «—»', () => {
    render(<PortadaNegocio portada={base({ fechaCorte: '2026-08-19', variacionPct: null, incompleta: false })} />);
...
describe('PortadaNegocio — incompleta (K-09 / BL-J4)', () => {
  it('con una conexión caída: dice qué falta y NO dibuja la variación', () => {
    render(<PortadaNegocio portada={base({ fechaCorte: '2026-08-19', variacionPct: '-18.0', incompleta: true })} />);
  ...
  it('completa: no hay aviso', () => {
    render(<PortadaNegocio portada={base({})} />);
    expect(screen.queryByTestId('midia-portada-incompleta')).not.toBeInTheDocument();
  });
});
