 ...
 * gasté en nafta este mes?»). Item #5 del contrato de Inteligencia de Negocio, §3.3.
 *
 ...
 * pregunta → «pensando» → respuesta. Reusa los primitivos de vidrio (`Burbuja`, `Composer`) para verse
 * igual, pero su máquina de estado es de una sola ida y vuelta, no el loop durable — mezclarlos sería
 * arrastrar la maquinaria del chat permanente a algo que no la necesita.
 *
 * 🔴 **VIVO desde PR #73** (`listo_backend-a-todos_IN-chat-VIVO...`, verificado contra el vivo con un
 * tenant efímero). La forma `{respuesta, fuente}` está congelada; `fuente` es metadata que esta
 * pantalla NO muestra — el guardarraíl de "nunca inventa" y "rechaza una acción" ya vive en el TEXTO
 * de `respuesta` (verificado E2E: pedir una acción devuelve la redirección al copiloto en el texto
 * mismo), así que no hace falta ramificar la UI sobre `fuente` para cumplir el DoD.
 *
