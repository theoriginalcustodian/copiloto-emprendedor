...
def test_save_get_roundtrip(crypto, conn_de_tenant):
    cid = str(uuid.uuid4())
...
def test_update_rotates_pair(crypto, conn_de_tenant):
    cid = str(uuid.uuid4())
    store = MpCredentialStore(conn_de_tenant(cid), cid, crypto)
...
def test_tokens_are_encrypted_at_rest(crypto, conn_de_tenant):
    cid = str(uuid.uuid4())
    store = MpCredentialStore(conn_de_tenant(cid), cid, crypto)
...
def test_adversarial_cross_tenant_isolation(crypto, conn_de_tenant):
    """A guarda; B (otro cliente_id) NO puede leer la credencial de A.

    Ahora hay DOS barreras y el test las cruza las dos: el filtro `cliente_id` explícito del store, y
    el RLS de la base — la conexión de B nace declarando B, así que la fila de A le es invisible
    ...
    store_b = MpCredentialStore(conn_de_tenant(cid_b), cid_b, crypto)
