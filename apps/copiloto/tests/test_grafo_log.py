...
@necesita_pg
def test_cobro_registrar_loggea_una_vez_y_el_replay_no_duplica(conn_de_tenant, tenants):
    a, _ = tenants
    cf = conn_de_tenant(a)
    comp_id = _comprobante_directo(cf, a, "1000.00")
    store = CobroStore(cf, a)
    store.registrar(comp_id, monto="400.00", medio="efectivo", idem_key="k1")
    ev = _eventos(cf, a, entidad_tipo="cobro")
    ...
    store.registrar(comp_id, monto="400.00", medio="efectivo", idem_key="k1")
    assert len(_eventos(cf, a, entidad_tipo="cobro")) == 1


@necesita_pg
def test_cobro_suelto_y_correccion(conn_de_tenant, tenants):
    a, _ = tenants
    cf = conn_de_tenant(a)
    store = CobroStore(cf, a)
...
@necesita_pg
def test_concepto_precio_de_a_y_volver_al_anterior_es_evento_nuevo(conn_de_tenant, tenants):
    a, _ = tenants
    cf = conn_de_tenant(a)
    store = ConceptoStore(cf, a)
    c = store.crear("Tablero", precio_referencia="85000")
    ev = _eventos(cf, a, entidad_tipo="concepto")
    assert len(ev) == 1 and ev[0]["evento"] == "creado"
    ...
    assert len(_eventos(cf, a, entidad_tipo="concepto")) == 2
...
@necesita_pg
def test_gasto_crear_e_imputacion(conn_de_tenant, tenants):
    a, _ = tenants
    cf = conn_de_tenant(a)
    gasto = GastoStore(cf, a).crear(monto=Decimal("8500"), categoria="transporte",
                                    proveedor="YPF", medio_pago="efectivo", origen="voz")
    ev = _eventos(cf, a, entidad_tipo="gasto")
    assert len(ev) == 1 and ev[0]["evento"] == "creado"
...
@necesita_pg
def test_presupuesto_crear_y_estado_idempotente(conn_de_tenant, tenants):
    a, _ = tenants
    cf = conn_de_tenant(a)
    store = PresupuestoStore(cf, a)
    p = store.crear(concepto="Instalación", receptor={"nombre": "Panadería"},
                    items=[{"descripcion": "tablero", "cantidad": 1, "precio_unitario": 85000}])
    ev = _eventos(cf, a, entidad_tipo="presupuesto")
    ...
    store.cambiar_estado(p["id"], APROBADO)
    assert len(_eventos(cf, a, entidad_tipo="presupuesto")) == 2


@necesita_pg
def test_afip_emitir_dedup_por_upsert_y_anular(conn_de_tenant, tenants):
    a, _ = tenants
    cf = conn_de_tenant(a)
    store = AfipComprobanteStore(cf, a)
    ...
    store.registrar(**kw)
    ev = _eventos(cf, a, entidad_tipo="comprobante")
    ...
    store.registrar(**kw)
    assert len(_eventos(cf, a, entidad_tipo="comprobante")) == 1
    ...
    store.marcar_anulada(cuit="30712345678", tipo_cbte=6, punto_venta=1, nro=7, nro_nota_credito=8)
    assert len(_eventos(cf, a, entidad_tipo="comprobante")) == 2


@necesita_pg
def test_aislamiento_cross_tenant_y_pares_de_ontologia(conn_de_tenant, tenants):
    a, b = tenants
    cf_a, cf_b = conn_de_tenant(a), conn_de_tenant(b)
    GastoStore(cf_a, a).crear(monto=Decimal("100"), categoria="otros")
    GastoStore(cf_b, b).crear(monto=Decimal("200"), categoria="otros")
    ev_a = _eventos(cf_a, a)
    ev_b = _eventos(cf_b, b)
