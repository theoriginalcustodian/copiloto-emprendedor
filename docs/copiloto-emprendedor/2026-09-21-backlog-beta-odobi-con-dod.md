...
| BL-V22 | **Bloqueo real del push de secretos (server-side).** A4-bis probó con un push REAL que con `core.hooksPath` apuntado a otro árbol git **no invoca** `.githooks/pre-push`: el commit con secreto sintético entró al remoto (caso 1c, rc=0; control 1d limpio también entró, así que los rechazos de 1a/1b prueban algo). El repo es **público** y GitHub tiene `secret_scanning` y `secret_scanning_push_protection` **disabled** (verificado por API 2026-09-22) | **No es un bug del hook.** El hook está bien escrito y es fail-closed; el techo es de la capa: ningún hook local puede defenderse de que git no lo invoque. Detección ya cubierta por dos capas independientes (`scripts/gate.sh:105-111` #649, con control negativo; y `scripts/vigilancia-check.sh:87-105` en el cron de 3 min, con test de 3 casos) — lo que falta es **bloqueo**, que sólo puede ser server-side. Es gratis en repos públicos. **Escalado al operador** (Telegram 2026-09-22, msg 25): el `PATCH` lo bloquea el clasificador de permisos. **DoD:** push protection habilitada + re-corrida del fixture 1c contra el remoto real → rechazado por el servidor | A4-bis ítem 1 (`a4bis-h1-push-y-gate.log`, refs `rama-a2 rama-b2`) · el Cierre A afirmaba «ya no apaga el hook en los demás worktrees», y **esa frase es falsa**: 1b prueba otra cosa (que el segundo worktree hereda bien el hook cuando el `hooksPath` es relativo) |
| BL-V23 | **Google Calendar reporta dos estados contradictorios en la misma sesión:** Mi día dice «se **cayó** la conexión… reconectala» y Agenda dice «**conectá** Google Calendar en Ajustes → Apps». Mismo tenant, misma sesión | No es cosmético: FE2 cerró la fila `agenda` de su matriz apoyándose en «el tenant no tiene Calendar vinculado», y Mi día afirma lo contrario — una fila cerrada sobre una premisa que puede ser falsa. **Medición pedida a auditoría (M-1, 2026-09-22):** cuál refleja el estado real y qué fuente lee cada pantalla. Si las dos leen lo mismo y difieren, ese es el bug. Dueño después de la medición | A4-bis H-BIS-2 |
| BL-V24 | **La latencia de `pres-hitl` no es una constante:** 5 s con el chat limpio, >151 s (sin respuesta) con propuestas pendientes y una hoja abierta | Cambia el fix, y por eso no es un duplicado de la fila de FE1: el «timeout ≥120 s» que ella declaró describe el **peor caso**, no el comportamiento. Si el disparador es el estado pendiente del chat, **subir el timeout trata el síntoma** — hay que medir primero qué hace el backend con propuestas abiertas | A4-bis H-BIS-4 · contrasta con el hallazgo de latencia de la matriz de FE1 |
...
| BL-V26 | (higiene, no es defecto de la app) «LO PEDISTE VOS» en prod muestra dos feedbacks de instrumentación entre los del usuario: «[e2e BL-W3 2026-09-21T18:15:39.347Z] prueba de evidencia PWA» | Contaminación de nuestras propias corridas de evidencia sobre datos que ve el usuario. Decidir si se borran de prod (muta prod: necesita autorización) o si las corridas futuras marcan sus escrituras para poder excluirlas | A4-bis, sección de higiene |
