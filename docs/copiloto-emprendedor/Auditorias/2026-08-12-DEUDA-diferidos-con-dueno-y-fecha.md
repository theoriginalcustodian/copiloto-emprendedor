...
D17                        | backend            | proxima corrida de smoke_beta_e2e.py        | abierto
```
<!-- DEUDA-VIVA:FIN -->
...
| D17 | **El smoke de prod se reportó como `34/35` y nadie escribió cuál es el 1 que falla.** El informe G8 declara **G6 ✅** citando ese número; buscado en `origin/main`, **no existe en todo `docs/` una sola salida con `fail=1` ni el nombre del check**. No es un problema del instrumento: `deploy/copiloto/smoke_beta_e2e.py:412-413` **ya imprime** `FALLARON: <nombres>` cuando hay rojos — el dato existió en la corrida y se recortó al citarlo. El veredicto `BETA-READY` es correcto igual (sólo exige los 5 de `CRIT`, `smoke_beta_e2e.py:404-407`), así que el fallo es **no crítico por construcción** | auditoría del informe de cierre (planificación, 2026-08-13) | backend | **próxima corrida de `smoke_beta_e2e.py`** — pegar la línea `FALLARON:` cruda, no el conteo | **Un rojo sin nombre no puede tener dueño ni disparador, y en tres semanas nadie va a poder decir si sigue rojo o se arregló solo.** Es exactamente el estado que el DoD §0 llama «Documentado» y niega como terminal. Cuesta una línea de copiar y pegar |
