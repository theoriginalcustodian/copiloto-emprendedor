...
D4                         | backend            | 1er sprint post-beta                        | cerrado
...
|---|---|---|---|---|---|
| D1 | ~~**C7** — Composio síncrono sin cache, 5 call-sites. `TTLCache` 30-60s per-tenant~~ — ✅ **CERRADO 2026-08-13, backend, #450** | P2 H-5 | backend | ✅ resuelto — TTL cache per-tenant (45s, reloj inyectable) en el choke point `ComposioGateway.list_connections`, cubre los 4 call-sites de lectura de un solo cambio + invalidación en disconnect. 7 tests nuevos, suite completa 1826/26-skip verde (VPS), CI 6/6, deploy corrido | Dict hand-rolled (no `cachetools`) por simplicidad + reloj inyectable para tests sin sleeps reales |
| D2 | ~~**C3** — fallo del Doc de presupuesto se loguea pero no va a la DLQ (no reintentable)~~ — ✅ **CERRADO 2026-08-13, backend, #447** | P2 H-6 | backend | ✅ resuelto | Ver PR #447 |
| D3 | ~~`heartbeat_timeout` ausente en las activities del loop ReAct (asimetría vs. AFIP, que sí lo tiene)~~ — ✅ **CERRADO 2026-08-13, backend, sin cambio de código** | P2 H-7 | backend | ✅ resuelto — hallazgo negativo verificado por código: ninguno de los ~20 `execute_activity` de `conversation_workflow.py` califica por el propio criterio de selección de AFIP (activity larga de una sola llamada bloqueante); todos ya están acotados a ≤120s por `ACTIVITY_TIMEOUT`/`MEMORY_TIMEOUT`, y ninguna instrumenta `activity.heartbeat()`. Aplicar `heartbeat_timeout` sin esa instrumentación es el anti-patrón documentado (falla la activity aunque esté sana) | Reportado al buzón (`2026-08-13_dato_backend-a-todos_D3-...`); no era un fix de línea, era confirmar que no aplicaba |
| D4 | ~~`patched()` sin gate de replay en CI~~ — ✅ **CERRADO 2026-08-13, backend, ADR-003** | P2 H-7 | backend | ✅ resuelto — el mecanismo (`temporalio.worker.Replayer` + fixtures reales) ya existía ad-hoc por-patch (2 tests). Se generalizó a un gate único (`apps/copiloto/tests/test_workflow_replay_gate.py`) con manifest + descubrimiento dinámico de todo `@workflow.defn` del repo + allowlist obligatoria con motivo — un workflow nuevo sin decisión de cobertura rompe el build. Cobertura real: 2/9 workflows (`ConversationWorkflow`, `FacturaWorkflow`, los de mayor riesgo); los otros 7 quedan en la allowlist con motivo, no en silencio | Ver `docs/copiloto-emprendedor/adr/2026-08-13_ADR-003_gate-de-replay-para-workflows-sin-patch.md` §5 para las limitaciones honestas (1 fixture por workflow no cubre toda la variedad de un ReAct) |
...
| D11 | ~~**Los 2 adversariales de C3 (lote C) NO aíslan el guard app-side**~~ — ✅ **CERRADO 2026-08-13, backend** | Fase D lote C (auditoría, 2026-08-12) | backend + auditoría | ✅ resuelto — rol de test `copiloto_aislamiento_probe` (`BYPASSRLS`+write, `deploy/copiloto/test-db.sh`), 3 UPDATE de los 2 archivos (`ConceptoStore.editar`, `CobroStore.completar`, `TrabajoStore.imputar`) revertidos: **caen los 3** sin el guard app-side (mismo rol BYPASSRLS); restaurado el filtro, **pasan los 3** con RLS siguiendo bypasseada — recién ahí el control aísla la capa interna, no el sistema completo | Ver abajo |
...
## D11 — RLS `FORCE` enmascara el control negativo de los adversariales C3 — ✅ CERRADO 2026-08-13

**Cierre.** Rol de test dedicado `copiloto_aislamiento_probe` (`BYPASSRLS`+write, radio acotado a
las 7 tablas que los 2 adversariales ejercitan de punta a punta), provisionado en
`deploy/copiloto/test-db.sh` — no en `deploy/worker/provision_tables.py` como decía el disparador
original: ese archivo es infra de tablas de PRODUCCIÓN, y el rol de este experimento es sólo-test,
mismo patrón que `copiloto_autosanacion`/`copiloto_consola` ya usan en el mismo script. Control
diferencial (`rolsuper rolbypassrls` = `false true`) antes de usarlo, mismo criterio que el resto de
los roles de ese script.

Con ese rol se revirtió el filtro `WHERE cliente_id` de los 3 `UPDATE` que los 2 adversariales
ejercitan (`ConceptoStore.editar` → `copiloto_conceptos`, `CobroStore.completar` → `copiloto_cobros`,
`TrabajoStore.imputar` → `copiloto_gastos` — el tercero no estaba nombrado en el disparador original
pero es el mismo archivo `test_cobros_y_catalogo.py` y el mismo patrón de `UPDATE`, así que se incluyó):

- **Rojo sin el guard** (rol `BYPASSRLS`, filtro sacado): los 3 tests **cayeron**, cada uno en la
  aserción exacta del `UPDATE` bajo prueba — A editó/completó/imputó lo de B.
- **Verde con el guard restaurado** (mismo rol, RLS sigue bypasseada): los 3 tests **pasaron** —
  prueba que el filtro app-side por sí solo, sin la red de RLS `FORCE` debajo, aísla los tenants.
- Suite completa de backend con el rol normal (`copiloto_app`, RLS aplicando): **1901 passed, 26
  skipped**, sin regresiones — los 2 archivos de store quedaron bit a bit idénticos a como estaban
  (`git diff` vacío tras revertir el experimento).

No se agregó ningún test permanente nuevo: el DoD de este ítem es una **demostración**, no una
aserción que viva rota en el repo entre corridas (el experimento requiere editar código de producción
a propósito, que es exactamente lo que NO debe quedar commiteado). Lo que queda permanente es el rol
+ su infra de provisioning, para que la próxima vez que se toque RLS `FORCE` o una policy, repetir
este experimento sea un comando, no un diseño desde cero.

<details><summary>Nota histórica (pre-cierre)</summary>
