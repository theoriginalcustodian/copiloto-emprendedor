...
| D7 | ~~`except: return False` en `mercadopago_gateway.py:119` — fail-silent~~ — **CERRADO 2026-08-13 00:00** | **P2** H-4 por sí solo, pero **P1 como instancia de D-A** (Pasada 2 H-3) — corregido 2026-08-12 20:47 | backend | resuelto | PR #424 (`3a75285a`, mergeado y **desplegado a prod**): `log_error_evento()` (fingerprint + `reason` del SDK) antes del `return False`, sin cambiar el fail-closed. `tests/test_mp_gateway.py` 7/7 en VPS. **Control negativo corrido a mano**: revertir el log hace caer exactamente los 2 tests nuevos, los otros 5 siguen verdes — confirma que miden el fix, no que pasan con o sin él. `scripts/gate.sh` 6/6 OK, recibo `.ci-recibos/3a75285a....json`. D-A queda 5/5: **desbloquea G2, G3 y G8** |
