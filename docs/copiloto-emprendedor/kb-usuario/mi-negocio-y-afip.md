# Mi negocio y tus datos fiscales ARCA

## Qué es y para qué sirve

Acá vive todo lo que el Copiloto necesita saber sobre tu negocio: qué vendés, cómo querés que te hable, y —lo más importante— tus datos fiscales para poder facturar por vos ante ARCA. Son dos cosas distintas que a veces se confunden: **Mi negocio** es el perfil comercial (qué vendés, tu tono de charla), y **Facturación ARCA** es el trámite fiscal (CUIT, condición de IVA, vínculo con ARCA). Cargar bien ambos es lo que hace que el Copiloto pueda facturar en tu nombre sin pedirte los mismos datos una y otra vez.

Se llega desde **Ajustes → Mi negocio** y **Ajustes → Facturación ARCA**, son dos pantallas separadas.
...
Importante: **acá no se cargan tu CUIT, razón social, domicilio ni condición frente al IVA** — esos datos fiscales van aparte, en Ajustes → Facturación ARCA.
...
## Facturación ARCA: tus datos fiscales
...
- **Producción** — comprobantes fiscales reales, con validez legal ante ARCA.
...
Este mensaje aparece si intentás facturar sin haber completado el perfil fiscal (Paso 1). Andá a Ajustes → Facturación ARCA y completá los campos que falten.
...
Fijate en Facturación ARCA, en la sección de ambientes: el que dice "Activo" es el que se está usando en este momento para facturar. Si tenés dudas antes de emitir una factura real, el botón de emisión en la pantalla de Facturación también te lo aclara — dice explícitamente "Emitir factura real" cuando estás en Producción.
...
No queda a medias. El Copiloto genera tu certificado con ARCA en un solo trámite automático; si algo se corta en el medio, el sistema lo detecta y te lo hace saber con un mensaje claro en vez de dejarte en un estado ambiguo. Si ya tenías una cuenta funcionando y un reintento posterior falla, tu vinculación anterior sigue activa — no se pierde por un intento fallido de actualizarla.

### ¿Por qué me pide usuario y clave fiscal si yo ya tengo Clave Fiscal en la web de ARCA?

Es el mismo usuario y clave que usás para entrar al portal de ARCA por tu cuenta. El Copiloto los necesita una única vez para hacer el trámite de vinculación en tu nombre — no crea una cuenta nueva ni un usuario distinto del que ya tenés con ARCA.

### Mi certificado con ARCA venció, ¿qué hago?
...
No. Los cargás una sola vez en Ajustes → Facturación ARCA y quedan guardados para todas las facturas futuras.

**¿El Copiloto guarda mi clave fiscal de ARCA?**
...
Todavía no. Podés cargar tu condición fiscal como Responsable Inscripto en Ajustes → Facturación ARCA, pero la Factura A o B con desglose de IVA no está disponible por el momento: hoy el Copiloto solo emite Factura C.

**¿Qué pasa si mi negocio tiene más de un punto de venta?**
Hoy la app admite un solo punto de venta por cuenta, el que cargás en Ajustes → Facturación ARCA. No hay forma de cargar ni elegir entre varios.
