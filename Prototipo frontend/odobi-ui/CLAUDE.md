...
Reordenado el **30/08/2026** con la estructura que usan los DESIGN.md de sistemas maduros
(ver `references/design-md/`). Antes estaba ordenado **por fecha de decisión**: servía para
reconstruir la historia, no para encontrar una regla. El color vivía en cuatro secciones
distintas y las reglas de método estaban numeradas 1, 2, 3.bis, 3, 4, 5, 5.bis, 6, 8, 7.

- **Partes 1 a 10 — el sistema de diseño.** Lo que aplica a cualquier pantalla.
- **Partes 11 y 12 — el producto.** Cómo está armada la app y qué dice el repo de David.
- **Partes 13 a 16 — cómo se trabaja.** Método, dónde vive cada archivo, cuándo algo está listo.

⚠️ **Nada se resumió ni se reescribió: las secciones están verbatim, movidas de lugar.** Lo
único que cambió es el ORDEN de las reglas de método, que ahora van 1 a 11 de corrido.

⚠️ **Cada decisión conserva su fecha.** Es lo que permite saber cuál deroga a cuál cuando dos
se contradicen — que pasa, y es normal en dos meses.

---

## 1 · Overview

Qué es Odobi, qué gobierna este archivo y qué no se reabre.
...
La paleta está cerrada desde el 22/07. ⚠️ **Todo par nuevo se calcula ANTES de usarse**, nunca a ojo.
...
Dos familias, cuatro tamaños por pantalla. Nada más.
...
Dos gramáticas visuales, y cuál le toca a cada pantalla.
...
## 5 · Elevación y superficie

De dónde sale la profundidad ahora que no hay bordes de 1 px.
...
## 6 · Formas e identidad

El símbolo, los íconos y la ilustración spot.
...
Cada pieza con la decisión que la sostiene y la alternativa descartada.
...
#### Chips de fila: el ORIGEN no se dice, lo que FALTA sí (25/08)
En Ingresos cada fila traía un chip de procedencia — "De una factura", "Lo dictaste por voz",
"Lo anotaste vos". **Se van los tres** (Martin): "De una factura" al lado de una fila que ya
dice *"Factura A-0036"* es decirlo dos veces, y **al usuario le da igual por qué puerta entró
el dato**. Sobrevive sólo el chip de lo que **falta** ("Falta de quién"), porque eso sí pide
una acción.
- ⚠️ **Y ese pierde el relleno:** era un pill negro sobre fila blanca y **pesaba más que el
  importe**, que es lo que la fila viene a mostrar. Pasa a texto en `#B04A2E` (5,43:1 ✅).
  **Un dato que falta se dice con texto, no con una mancha.**
- No confundir con el **chip de prioridad** de Mi día ("Lo anotaste vos" en `c-sin`): ése no
  dice origen, dice **cuánto apura**. Distinto trabajo, se queda.

#### El aviso "Guardado" abre y cierra su propio espacio (25/08)
Antes sólo se **desvanecía**: `opacity` a 0 con el elemento todavía ocupando su alto. El campo
quedaba con un **hueco permanente** y la lista se veía despareja — los campos tocados con más
aire que los intactos, sin ninguna razón visible. Ahora se anima `max-height` + `margin-top`
junto con la opacidad: el aire **aparece con el mensaje y se va con él** (medido: 85 → 103 → 85 px).
- ⚠️ **Fallback obligatorio con `prefers-reduced-motion`.** El `*{animation:none!important}`
  mataría el keyframe y el aviso **se quedaría en `max-height:0`, o sea invisible**. La
  confirmación de que un dato se guardó **no es decoración**: se muestra igual, sin transición.
- Los 14 avisos llevan `role="status" aria-live="polite"`: ahora que el elemento vive siempre
  en el DOM, corresponde que se anuncie.
- ⚠️ **Cómo se verifica una animación en headless:** `--virtual-time-budget` la **completa al
  instante**, así que medir "durante" siempre devuelve el estado final. Se congela con
  `animation:<nombre> <dur> -<t>s paused` y se mide ahí.

#### Los campos del HITL van MÁS SUELTOS que una lista (25/08)
`.hitl .kv` pasa de 6 px a **14 px** entre filas, con la subetiqueta a 4 px de su valor y la
nota de alcance a 14 del último campo.
**El fundamento no es estético:** en el HITL cada fila es un dato que hay que **leer y
verificar** antes de confirmar algo que no se deshace — no un ítem que se escanea. Con 6 px, la
subetiqueta de un campo ("CUIT 20-…") quedaba tan cerca del rótulo del siguiente que se leían
como un bloque solo.
⚠️ **Lo que ordena es la PROPORCIÓN, no el número:** el espacio entre campos tiene que ser mayor
que el que separa cada rótulo de su valor. Eso es lo que hace que se lean de a pares (Gestalt),
y es lo mismo que rige el aire de los rótulos de sección (32 arriba / 22 abajo).

#### Ayuda: tres opciones, TRES SUPERFICIES (30/08)
Se pidieron como "tres chats" y no son lo mismo: cambia **con quién hablás**, **cuándo te
contesta** y **qué forma le corresponde**.

| Opción | Con quién | Cuándo | Forma |
|---|---|---|---|
| Cómo usar la app | **Odobi** | al instante | temas → **el chat general** |
| Soporte técnico | **asistente de soporte** | al instante, escala a ticket | **chat PROPIO** |
| Contanos qué tal | el equipo | **no contesta, y está bien** | **formulario** |

- ⚠️ **"Cómo usar la app" NO abre un cuarto chat.** Cada tema entra al chat principal con
  contexto (puente de la Decisión C). Sería la duplicación que ya sacamos con "Preguntar" de
  Inteligencia. **El chip de contexto es la PUERTA DE VUELTA**, no una etiqueta: sin eso hacen
  falta cuatro toques para el tema siguiente.
- ⚠️ **El avatar de Soporte NO es el isotipo.** El avatar es lo primero que se lee de un
  mensaje: si fuera el de Odobi, la burbuja diría que contesta el copiloto. **El copiloto sabe
  de tu negocio; de sus propios bugs, no.**
- ⚠️ **El composer de Soporte lleva CLIP, no mic.** El mic es **el gesto de Odobi**, y un
  problema técnico **se muestra mejor de lo que se cuenta**. Usa `.barra-sop` y no `.inputbar`:
  `armarVoz()` corre sobre **todos** los `.inputbar` y le habría enganchado la grabación.
- **El ticket vive EN el hilo.** Lo que se decidió queda donde se decidió, igual que el
  comprobante del HITL.
- **Feedback: campo libre, sin clasificar.** Nada de chips de tipo: clasificar es trabajo que
  se le pasa al usuario, y en feedback **la fricción se paga en silencio** — el que dudaba, no
  escribe. ⚠️ El problema de esa pantalla **no es estético**: un formulario que además avisa que
  no contesta **parece un buzón sin fondo**. Se resuelve con una **pregunta concreta** en vez de
  una caja muda, y con **"Lo pediste vos"** —tres cosas ya construidas, con su mes—, que es lo
  que convierte "no lleva respuesta" de excusa en política.

⚠️ **Divergencias con `kb-usuario/ajustes.md`:** sólo **Feedback** existe en el repo, y vive
**dentro de Mi cuenta**. **Soporte técnico y Cómo usar la app son VISIÓN** — el repo resuelve
soporte con *"escribinos"*, un canal externo.

#### "No molestar": una fila que existe y NO HACE NADA (30/08)
El repo la define así de literal: *"la fila queda visible pero sin hacer nada al tocarla"*. Vive
en **Mi cuenta**, junto a Feedback y Cerrar sesión.

> **Regla: si no hace nada, no puede parecer que lo hace.**

- Se le sacan el **chevron**, el **cursor** y el contraste pleno del título; se le pone un chip
  **"Pronto"**. Son las tres señales de "acá pasa algo al tocar".
- ⚠️ **NUNCA un switch apagado.** El usuario lo toca, no se mueve, y ahí sí concluye que la app
  falla. **Un control muerto miente más que una etiqueta.**
- **El motivo NO es relleno.** *"Pronto"* a secas es una promesa; con el porqué es una
  explicación — y **el cuidado es la razón de la demora**, así que decirlo convierte una
  carencia en una señal de criterio.
- Contrastes: chip `sec` s/`arena-30` **6,04:1** ✅ · título atenuado s/blanco **7,51:1** ✅.

---

## 8 · Movimiento

Dónde hay animación, cuánta y por qué.
...
Cómo habla Odobi, y cómo se le habla a Odobi.
...
Lo que no se hace, siempre con el motivo.
...
Navegación, capas y superficies.
...
Lo verificado contra `kb-usuario/`, y las divergencias que esperan a David.
...
## 13 · Método de edición

⚠️ Cada regla nace de algo que ya se rompió acá. No son buenas prácticas genéricas.
...
#### 4 · Un elemento puede destruirlo la propia función que lo usa
...
⚠️ **Es la regla 3 con una vuelta más:** el elemento no quedó huérfano por un refactor —
...
#### 4.bis · Los márgenes verticales entre hermanos COLAPSAN
Una separación que "no se aplica" sin razón aparente casi siempre es esto: entre hermanos gana
el mayor de los dos márgenes, no la suma. **Se resuelve con `padding`, que no colapsa.** Caso
real: la advertencia de dato incompleto quedaba pegada al importe porque su `margin-top` lo
absorbía el margen de la cifra.

#### 5 · Verificar en ALTO REDUCIDO, no sólo en el marco de 844
...
#### 6 · Verificar el CONTENIDO del render, no su peso
...
#### 7 · Los mockups se renderizan por HTTP, nunca por `file://`
...
#### 8 · No recrear lo que ya existe como fuente
...
## 14 · Prototipo, mockups y assets

Dónde vive cada cosa y cómo se regenera.
...
Autoevaluar ANTES de mostrar. Si un punto falla, se corrige primero.
...
## 16 · Guía de iteración

Qué está cerrado, qué sigue abierto y qué depende de terceros.
