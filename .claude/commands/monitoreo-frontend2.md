---
description: Arranque VERIFICADO de la sesión FRONTEND-2: cron + harness de buzón + contexto + qué arranca, con reporte binario
allowed-tools: CronList, CronCreate, Read, Bash, Glob, Grep
---

# Arrancar el monitoreo de la sesión FRONTEND-2

Instalá el cron de heartbeat de ESTA sesión (frontend). **Corré este comando EN LA VENTANA DE
FRONTEND-2** — un cron no se puede crear para otra sesión.
...
1. **`CronList`** — mirá si ya existe un cron con schedule `*/3 * * * *` cuyo prompt arranque con
   "Vigía de coordinación (sesión FRONTEND2)". Si ya está → no crees nada, saltá al paso 3.
...
5.bis. **Instrumento** — `SESION_ACTUAL=frontend2 bash scripts/vigilancia-check.sh --quiet`. Corré esto primero en cada tick del
...
6. **Buzón** — listá `coordinacion/abierto/` filtrando `-a-frontend2_`, `-a-frontend_` (broadcast a las dos) y `-a-todos_`, **y también**
...
```
✅/❌ cron FRONTEND-2 vivo (schedule */3, próximo tick HH:MM)
...
> Contexto: sesión FRONTEND-2 del trabajo en sesiones paralelas (planificación/backend/frontend1/frontend2/
...
## Cron — Vigía de coordinación (FRONTEND-2)

- **Schedule (cron):** `*/3 * * * *`  (cada 3 minutos)
- **Prompt:**

```
Vigía de coordinación (sesión FRONTEND2).
...
0. 🔴 GATE DETERMINISTA (chequeo GLOBAL, no reemplaza el paso 1) — corré primero:
   `SESION_ACTUAL=frontend2 bash scripts/vigilancia-check.sh --quiet`
...
1. Listar `abierto/` y quedarte SÓLO con `-a-frontend2_`, `-a-frontend_` y `-a-todos_`. Descartar lo que empiece por
   `frontend2-a-` (es tuyo). Un `-a-frontend1_` es de la OTRA sesión: no lo tomes. Mirar también `cerrado/<hoy>/` por los `avance_` y `dato_`, que nacen
